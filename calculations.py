...
class Calculations(ParseCSV):
    ...
    def get_movie_with_year_data(self, column1, column2, year):
        movie_with_year_data = [(movie_with_year_data[column1], movie_with_year_data[column2])
    ...
    def movie_with_year(self, year):
        return self.get_movie_with_year_data('originalTitle', 'rating', year)
    ...
    def movie_with_avg_runtime(self, year):
        movie_with_year_data = self.get_movie_with_year_data('originalTitle', 'runtimeMinutes', year)
        return mean([_tuple[1] for _tuple in movie_with_year_data])

    def top_n_highest_rated(self, year):
        movie_with_year_data = self.get_movie_with_year_data('originalTitle', 'numVotes', year)
        movie_with_year_data.sort(key=operator.itemgetter(1))
    ...
    def store_results_of_first_argument(self):
        ...
        self.result_dictionary['-r'] = year_rating

    def store_results_of_second_argument(self):
        if not self._args.genre:
            return None
        ...
        self.result_dictionary['-g'] = genre_rating

    def store_results_of_third_argument(self):
        if not self._args.year_rating:
            return None
        ...
        self.result_dictionary['-v'] = top_ratings
